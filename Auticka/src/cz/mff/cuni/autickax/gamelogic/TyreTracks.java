	...
	public void draw(SpriteBatch batch, float parentAlpha) {
		...
		for (int i = leftTyrePositions.size() - 1; i > 1; i--) {
			...
			if (drawLine) {
				...
				shapeRenderer.line(new Vector2(left1.position).scl(stretch),
						new Vector2(left2.position).scl(stretch));
				shapeRenderer.line(new Vector2(right1.position).scl(stretch),
						new Vector2(right2.position).scl(stretch));
			}
