...
public final class SwitchingMinigame extends Minigame {
	...
	private void createProgressBar(){
		...
		style.background = new TextureRegionDrawable(Autickax.getInstance().assets.getGraphics(Constants.minigames.SWITCHING_MINIGAME_SLIDER_BACKGROUND_TEXTURE));
		style.knob = new TextureRegionDrawable(Autickax.getInstance().assets.getGraphics(Constants.minigames.SWITCHING_MINIGAME_SLIDER_KNOB_TEXTURE));
		progressBar = new Slider(0, 100, 0.001f, false, style);
		progressBar.setDisabled(true);
		progressBar.setPosition(400 * Input.xStretchFactorInv - progressBar.getWidth() / 2, 420 * Input.yStretchFactorInv);
	...
	private void createButtons() {
		...
		style.up = new TextureRegionDrawable(Autickax.getInstance().assets.getGraphics(Constants.minigames.SWITCHING_MINIGAME_BUTTON_TEXTURE));
		style.disabled = new TextureRegionDrawable(Autickax.getInstance().assets.getGraphics(Constants.minigames.SWITCHING_MINIGAME_DISABLED_BUTTON_TEXTURE));
		for (int i = 0; i < buttons.length; i++) {
			SwitchingMinigameButton newButton = new SwitchingMinigameButton(style, this);
			newButton.setDisabled(true);						
			newButton.setPosition(Constants.dialog.DIALOG_WORLD_X_OFFSET * Input.xStretchFactorInv + Constants.dialog.DIALOG_WORLD_WIDTH * Input.xStretchFactorInv / 4 + Constants.dialog.DIALOG_WORLD_WIDTH * Input.xStretchFactorInv * i / 2 - newButton.getWidth() / 2,
					Constants.dialog.DIALOG_WORLD_Y_OFFSET * Input.yStretchFactorInv + Constants.dialog.DIALOG_WORLD_HEIGHT * Input.yStretchFactorInv / 2 - newButton.getHeight() / 2);
