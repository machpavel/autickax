...
public final class BoostMinigame extends Minigame {
	...
	private final float RESULT_WIN_VALUE = Constants.BOOST_MINIGAME_RESULT_WIN_VALUE;
	private final float TIME_GENERATION_LIMIT = Constants.BOOST_MINIGAME_TIME_GENERATION_LIMIT;	
	...
	public BoostMinigame(GameScreen screen, SubLevel parent) {
		...
		this.backgrountTexture = new TextureRegionDrawable(
				Autickax.getInstance().assets
						.getGraphics(Constants.BOOST_MINIGAME_BACKGROUND_TEXTURE));

		if (Autickax.settings.showTooltips)
			this.parent.setDialog(new MessageDialog(screen, parent,
					Constants.TOOLTIP_MINIGAME_BOOST_WHAT_TO_DO));
		...
		for (int row = 0; row < this.rowsCount; row++) {
			for (int column = 0; column < this.columnCount; column++) {
				BoostMinigameButton newWaitingButton = new BoostMinigameButton(this.columnCount * row + column, this, 
						Autickax.getInstance().assets
						.getGraphics(Constants.BOOST_MINIGAME_WAITING_BUTTON_TEXTURE));
				...
				BoostMinigameButton newHighlightedButton = new BoostMinigameButton(this.columnCount * row + column, this, 
						Autickax.getInstance().assets
						.getGraphics(Constants.BOOST_MINIGAME_HIGHLIGHTED_BUTTON_TEXTURE));
	...
	private void fail() {
		this.resultMessage = Constants.TOOLTIP_MINIGAME_BOOST_FAIL;
	...
	private void updateInFinishState(float delta) {
		this.resultMessage = Constants.TOOLTIP_MINIGAME_BOOST_SUCCESS;
	...
	private void setDifficulty(Difficulty difficulty) {
		switch (difficulty) {
		case Kiddie:
			this.rowsCount = Constants.BOOST_MINIGAME_ROWS_KIDDIE;
			this.columnCount = Constants.BOOST_MINIGAME_COLUMNS_KIDDIE;
			this.timeLimit = Constants.BOOST_MINIGAME_TIME_LIMIT_KIDDIE;
			this.hitsLimit = Constants.BOOST_MINIGAME_HITS_LIMIT_KIDDIE;
			break;
		case Beginner:
			this.rowsCount = Constants.BOOST_MINIGAME_ROWS_BEGINNER;
			this.columnCount = Constants.BOOST_MINIGAME_COLUMNS_BEGINNER;
			this.timeLimit = Constants.BOOST_MINIGAME_TIME_LIMIT_BEGINNER;
			this.hitsLimit = Constants.BOOST_MINIGAME_HITS_LIMIT_BEGINNER;
			break;
		case Normal:
			this.rowsCount = Constants.BOOST_MINIGAME_ROWS_NORMAL;
			this.columnCount = Constants.BOOST_MINIGAME_COLUMNS_NORMAL;
			this.timeLimit = Constants.BOOST_MINIGAME_TIME_LIMIT_NORMAL;
			this.hitsLimit = Constants.BOOST_MINIGAME_HITS_LIMIT_NORMAL;
			break;
		case Hard:
			this.rowsCount = Constants.BOOST_MINIGAME_ROWS_HARD;
			this.columnCount = Constants.BOOST_MINIGAME_COLUMNS_HARD;
			this.timeLimit = Constants.BOOST_MINIGAME_TIME_LIMIT_HARD;
			this.hitsLimit = Constants.BOOST_MINIGAME_HITS_LIMIT_HARD;
			break;
		case Extreme:
			this.rowsCount = Constants.BOOST_MINIGAME_ROWS_EXTREME;
			this.columnCount = Constants.BOOST_MINIGAME_COLUMNS_EXTREME;
			this.timeLimit = Constants.BOOST_MINIGAME_TIME_LIMIT_EXTREME;
			this.hitsLimit = Constants.BOOST_MINIGAME_HITS_LIMIT_EXTREME;
			break;
		default:
