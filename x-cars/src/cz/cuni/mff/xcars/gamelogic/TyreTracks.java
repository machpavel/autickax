...
public class TyreTracks extends Actor {
	...
	private final ShapeRendererStretched shapeRenderer;
	private Car car;
	...
	public TyreTracks(Vector2 objectPosition, Car car, IElapsed elapsed) {
		...
		this.shapeRenderer = new ShapeRendererStretched();
		this.car = car;
	...
	public void addPoint(Vector2 objectCenter, float time) {
		...
		if (dist <= Constants.tyreTracksConstants.MAX_SEGMENT_LENGTH || tyrePositions.isEmpty())
			...
			ArrayList<TyreTrackInfo> tri = getInterpolatedPositions(last.leftPosition, leftPos, last.rightPosition,
					rightPos, last.time, time, objectCenter);
	...
	private ArrayList<TyreTrackInfo> getInterpolatedPositions(Vector2 startLeft, Vector2 finishLeft,
			Vector2 startRight, Vector2 finishRight, float timeStart, float timeFinish, Vector2 prevCenter) {
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		for (int i = tyrePositions.size() - 1; i > 1; i--) {
			...
			boolean drawLine = distSquaredFromCenter > Constants.tyreTracksConstants.MAX_DIST_SQUARED
			...
			if (drawLine) {
				...
				shapeRenderer.line(new Vector2(trackInfo1.leftPosition), new Vector2(trackInfo2.leftPosition));
				shapeRenderer.line(new Vector2(trackInfo1.rightPosition), new Vector2(trackInfo2.rightPosition));
			}
		}
		shapeRenderer.end();
