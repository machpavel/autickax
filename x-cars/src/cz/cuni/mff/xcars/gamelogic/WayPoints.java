...
/**
 *	This class represents positions on the track
 *	that need to be crossed in order to successfully
 *	finish the race.
 */
public class WayPoints extends Actor{
	...
	public WayPoints(Finish finish,Start start, Pathway pathway)
	{
		...
		this.finish = finish;
		this.start = start;
		this.sumRadii = finish.getBoundingRadius() + Constants.misc.MAX_DISTANCE_FROM_PATHWAY;
	...
	public void initWayPoints(float start, float finish, int nofWayPoints) {
		...
		for (float f = start; f < finish; f += step) {
			...
			if (distToFinish2(pathPosition) >   sumRadii2  && distToStart2(pathPosition) > sumRadii2)
			{
	...
	public void draw(Batch batch, float parentAlpha)
	{
		...
		if (Debug.DEBUG && Debug.drawWayPoints)
		{
			...
			{
				this.renderer.circle(way.x * Input.xStretchFactorInv, way.y* Input.yStretchFactorInv,
						Constants.misc.MAX_DISTANCE_FROM_PATHWAY*Input.xStretchFactorInv);
